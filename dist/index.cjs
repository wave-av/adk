...
__export(index_exports, {
  ...
  AutoProducerAgent: () => AutoProducerAgent,
  CaptionAgent: () => CaptionAgent,
  ClipFactoryAgent: () => ClipFactoryAgent,
  ModerationAgent: () => ModerationAgent,
  StreamMonitorAgent: () => StreamMonitorAgent,
  WaveAgent: () => WaveAgent,
  createClipNode: () => createClipNode,
  createKernelTools: () => createKernelTools,
  createLangGraphTools: () => createLangGraphTools,
  createLiveKitWaveTools: () => createLiveKitWaveTools,
  createMastraTools: () => createMastraTools,
  createStreamMonitorNode: () => createStreamMonitorNode,
  createStreamMonitorStep: () => createStreamMonitorStep,
  createWaveMCPConfig: () => createWaveMCPConfig,
  createWaveStreamSource: () => createWaveStreamSource
});
...
var AgentToolkit = class {
  ...
  getTools() {
    const createStreamSchema = import_zod.z.object({ title: import_zod.z.string().min(1), protocol: import_zod.z.enum(["webrtc", "srt", "rtmp", "ndi"]).optional() });
    const streamIdSchema = import_zod.z.object({ streamId: import_zod.z.string().uuid() });
    const clipSchema = import_zod.z.object({ streamId: import_zod.z.string().uuid(), startTime: import_zod.z.number().min(0), endTime: import_zod.z.number().min(0) });
    const switchSchema = import_zod.z.object({ switcherId: import_zod.z.string(), sourceId: import_zod.z.string(), transition: import_zod.z.enum(["cut", "mix", "wipe"]).optional() });
    const graphicSchema = import_zod.z.object({ switcherId: import_zod.z.string(), templateId: import_zod.z.string(), data: import_zod.z.record(import_zod.z.unknown()).optional() });
    const moderateSchema = import_zod.z.object({ messageId: import_zod.z.string(), action: import_zod.z.enum(["approve", "flag", "block"]) });
    const captionSchema = import_zod.z.object({ streamId: import_zod.z.string().uuid(), language: import_zod.z.string().length(2).optional() });
    const analyticsSchema = import_zod.z.object({ streamId: import_zod.z.string().uuid(), timeRange: import_zod.z.enum(["1h", "24h", "7d"]).optional() });
    const highlightSchema = import_zod.z.object({ streamId: import_zod.z.string().uuid(), label: import_zod.z.string().min(1) });
    const cameraSchema = import_zod.z.object({ cameraId: import_zod.z.string(), pan: import_zod.z.number().min(0).max(1).optional(), tilt: import_zod.z.number().min(0).max(1).optional(), zoom: import_zod.z.number().min(0).max(1).optional() });
    return [
      {
...
function createKernelTools(config) {
  ...
  const browseSchema = import_zod2.z.object({ url: import_zod2.z.string().url(), waitForSelector: import_zod2.z.string().optional(), timeoutMs: import_zod2.z.number().optional() });
  const screenshotSchema = import_zod2.z.object({ url: import_zod2.z.string().url(), selector: import_zod2.z.string().optional(), width: import_zod2.z.number().optional(), height: import_zod2.z.number().optional() });
  const playwrightSchema = import_zod2.z.object({ code: import_zod2.z.string().min(1), url: import_zod2.z.string().url().optional() });
  return [
    {
      name: "browse_url",
